...
import { CheckSquare, Plus, RefreshCw, Users, List, X, AlertTriangle } from 'lucide-react';
...
export function TasksView() {
  ...
  const hasActiveFilters =
    (filterPerson !== null && filterPerson.length > 0) ||
    filterPriority !== null ||
    filterList !== null;
  const clearFilters = () => {
  ...
  const secondaryGroupOptions = useMemo((): FilterOption[] => {
    ...
    if (primaryGroup !== 'list' && taskLists.length > 0)
      ...
      <div className="flex h-screen flex-col">
        <SubpageHeader
          ...
          badge={
            <>
              ...
              {autoSyncing && <RefreshCw className="h-4 w-4 animate-spin text-muted-foreground" />}
            </>
          }
          actions={
            <>
              {pendingCount > 0 && (
                <Button
                  ...
                  className="h-9 border-amber-500/50 text-amber-600 hover:bg-amber-500/10 dark:text-amber-400"
        ...
        <FilterBar>
          ...
          {primaryGroup !== 'none' && taskLists.length > 0 && (
            <FilterDropdown
              ...
              onSelectionChange={(s) =>
                setSecondaryGroup((s.size > 0 ? [...s][0]! : 'none') as GroupBy)
              }
              mode="single"
            />
          )}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          <TaskContentArea
            ...
            tasksByPersonThenList={tasksByPersonThenList}
            tasksByListThenPerson={tasksByListThenPerson}
            inlineTask={inlineTask}
            setInlineTask={setInlineTask}
            inlineTaskByUser={inlineTaskByUser}
            setInlineTaskByUser={setInlineTaskByUser}
            inlineTaskByList={inlineTaskByList}
            setInlineTaskByList={setInlineTaskByList}
            ...
            handleAddWithAuth={handleAddWithAuth}
          />
        </div>
              ...
              try {
                const res = await fetch(`/api/tasks/${editingTask.id}`, {
                  ...
                  body: JSON.stringify({
                    ...
                    dueDate:
                      updatedTask.dueDate === null ? null : updatedTask.dueDate.toISOString(),
