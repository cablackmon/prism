...
export function NestedGroupedTaskGrid({
  ...
  const saveOrder = useCallback((order: string[]) => {
    setGroupOrder(order);
    try {
      localStorage.setItem('prism:task-nested-group-order', JSON.stringify(order));
    } catch {}
  }, []);
        ...
        {sortedGroups.map((group, idx) => {
          ...
          return (
            ...
            >
              ...
              {/* Sub-groups */}
              <div className="flex-1 space-y-2 overflow-y-auto overscroll-contain px-2 pb-2">
                ...
                {group.subGroups.map((sub) => (
                  <div key={sub.key}>
                    ...
                    <div
                      ...
                      style={{ borderColor: sub.color + '60' }}
                    >
                      {sub.tasks.map((task) => (
                        <TaskRow
                          key={task.id}
