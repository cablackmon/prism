...
export function useTasksViewData() {
  ...
  useEffect(() => {
    setTasks(
      apiTasks.map((t) => ({
        ...t,
        dueDate:
          t.dueDate instanceof Date ? t.dueDate : t.dueDate ? new Date(t.dueDate) : undefined,
      }))
    );
  }, [apiTasks]);
  ...
  const filteredTasks = useMemo(() => {
    ...
    if (filterPerson && filterPerson.length > 0) {
      result = result.filter(
        (task) => task.assignedTo?.id && filterPerson.includes(task.assignedTo.id)
      );
    }
    if (filterPriority) {
    ...
    if (filterList !== null) {
      if (filterList === 'none') {
        ...
        result = result.filter(
          (task) => (task as Task & { listId?: string }).listId === filterList
        );
      }
    }
    result.sort((a, b) => {
  ...
  const toggleTask = async (taskId: string): Promise<boolean> => {
    ...
    if (!isParent && !isAssignedToUser) {
      toast({
        title: `This task is assigned to ${task.assignedTo?.name}. Only they can mark it complete.`,
        variant: 'warning',
      });
      return false;
    }
  ...
  const editTask = async (task: Task) => {
    ...
    if (!user) return;
    if (user.role !== 'parent') {
      toast({ title: 'Only parents can edit tasks', variant: 'warning' });
      return;
    }
    setEditingTask(task);
  };
  ...
  const deleteTask = async (taskId: string) => {
    ...
    if (!user) return;
    if (user.role !== 'parent') {
      toast({ title: 'Only parents can delete tasks', variant: 'warning' });
      return;
    }
