...
import { ClipboardList, Plus, AlertCircle, History, Users, X } from 'lucide-react';
...
export function ChoresView() {
  ...
  const [addModalDefaultAssignee, setAddModalDefaultAssignee] = useState<string | undefined>(
    undefined
  );
  ...
  const hasActiveFilters =
    (filterPerson !== null && filterPerson.length > 0) || categoryFilters.size > 0;
  ...
  const choresByUser = useMemo(() => {
    ...
    effectiveFilteredChores.forEach((c) => {
      if (c.assignedTo) assigneeMap.set(c.assignedTo.id, c.assignedTo);
    });
    ...
    const groups: {
      ...
      chores: typeof effectiveFilteredChores;
    }[] = [];
      ...
      <div className="flex h-screen flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {showCompletions ? (
            ...
            <div className="flex h-full flex-col items-center justify-center text-destructive">
              ...
              <p>{error}</p>
              <Button variant="outline" size="sm" className="mt-4" onClick={() => refreshChores()}>
                Try Again
              </Button>
            </div>
        ...
        {showAddModal && (
          <ChoreModal
            ...
            defaultAssignedTo={addModalDefaultAssignee}
          />
        )}
        {editingChore && (
          <ChoreModal
