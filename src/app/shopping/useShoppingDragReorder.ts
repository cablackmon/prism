...
export function useShoppingDragReorder({
  ...
  const touchStartRef = useRef<{ x: number; y: number; element: HTMLElement | null }>({
    x: 0,
    y: 0,
    element: null,
  });
  ...
  const saveCategoryOrder = useCallback(
    (order: string[]) => {
      const reordered = order
        .map((id) => dynamicCategories.find((c) => c.id === id))
        .filter(Boolean) as typeof dynamicCategories;
