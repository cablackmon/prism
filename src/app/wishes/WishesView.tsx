...
export function WishesView() {
  ...
  const { items, loading, error, addItem, updateItem, deleteItem, claimItem, unclaimItem } =
  ...
  return (
    <div
      ...
      className={cn(
        ...
        'flex h-full min-h-0 flex-col overflow-hidden rounded-xl border-2 bg-card/50',
        'transition-all',
    ...
    >
      ...
      >
        ...
        <div className="h-3 w-3 shrink-0 rounded-full" style={{ backgroundColor: member.color }} />
        <h3 className="truncate text-sm font-semibold" style={{ color: member.color }}>
          {member.name}
        </h3>
        <span className="ml-1 whitespace-nowrap text-xs text-muted-foreground">{items.length}</span>
        <Button
          variant="ghost"
          size="icon"
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        {isOwnList && item.claimed && (
          <p className="mt-0.5 text-xs text-green-600 dark:text-green-400">Got it myself</p>
        )}
      </div>
