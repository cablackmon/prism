    ...
    <div className="mt-1 space-y-1">
      <div className="relative">
        ...
        <Input
          autoFocus
          placeholder={
            childType === 'national_park' ? 'Search national parks…' : 'Search city or place…'
          }
          value={query}
      ...
      {childType === 'stop' && results.length > 0 && (
        <div className="max-h-44 overflow-y-auto rounded-md border border-border bg-popover shadow-md">
          {results.map((r) => (
            <button
              key={r.placeId}
              disabled={adding}
            ...
            >
              ...
              {r.fullName && r.fullName !== r.displayName && (
                <div className="truncate text-[10px] text-muted-foreground">{r.fullName}</div>
              )}
            </button>
          ))}
        </div>
      ...
      {childType === 'national_park' && (
        <div
          className={cn(
            'max-h-44 overflow-y-auto rounded-md border border-border bg-popover shadow-md',
            !query && 'hidden'
          )}
        >
          {parkResults.length > 0 ? (
            parkResults.map((u) => (
              <button
                ...
                className="flex w-full items-center gap-1.5 border-b border-border px-2.5 py-1.5 text-left text-xs transition-colors last:border-0 hover:bg-accent"
              >
                <Emoji e="🌲" /> <span className="font-medium">{u.name}</span>
                <span className="ml-auto text-[10px] text-muted-foreground">{u.type}</span>
              </button>
            ))
          ) : (
