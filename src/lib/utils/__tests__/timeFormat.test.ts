...
describe('time format utilities', () => {
  ...
  it('persists calendar form times in the selected display timezone', () => {
    expect(fromDisplayDateTime('2026-08-19', '08:00', 'Europe/Warsaw').toISOString()).toBe(
      '2026-08-19T06:00:00.000Z'
    );
    expect(fromDisplayDateTime('2026-08-19', '08:00', 'Europe/London').toISOString()).toBe(
      '2026-08-19T07:00:00.000Z'
    );
  });

  it('uses the correct selected-timezone offset across daylight saving time', () => {
    expect(fromDisplayDateTime('2026-01-19', '08:00', 'Europe/Warsaw').toISOString()).toBe(
  ...
  it('keeps a Google all-day event on its date despite a positive timezone offset', () => {
    ...
    expect(
      eventOccursOnDisplayDay(start, exclusiveEnd, true, new Date(2026, 7, 18), 'Europe/Warsaw')
    ).toBe(true);
    expect(
      eventOccursOnDisplayDay(start, exclusiveEnd, true, new Date(2026, 7, 19), 'Europe/Warsaw')
    ).toBe(false);
  });
  ...
  it('supports Prism all-day events with an inclusive end-of-day timestamp', () => {
    ...
    expect(
      eventOccursOnDisplayDay(start, inclusiveEnd, true, new Date(2026, 7, 18), 'Europe/Warsaw')
    ).toBe(true);
    expect(
      eventOccursOnDisplayDay(start, inclusiveEnd, true, new Date(2026, 7, 19), 'Europe/Warsaw')
    ).toBe(false);
  });
  ...
  it('shows a timed event start only on its first displayed day', () => {
    ...
    expect(eventStartsOnDisplayDay(start, false, new Date(2026, 7, 21), 'Europe/Warsaw')).toBe(
      true
    );
    expect(eventStartsOnDisplayDay(start, false, new Date(2026, 7, 22), 'Europe/Warsaw')).toBe(
      false
    );
  });
  ...
  it('uses floating UTC dates for all-day event starts', () => {
    ...
    expect(eventStartsOnDisplayDay(start, true, new Date(2026, 7, 21), 'America/Los_Angeles')).toBe(
      true
    );
    expect(eventStartsOnDisplayDay(start, true, new Date(2026, 7, 20), 'America/Los_Angeles')).toBe(
      false
    );
  });
  ...
  it('keeps an ongoing timed event active until its actual end instant', () => {
    ...
    expect(isCalendarEventPast(start, end, false, new Date('2026-08-20T07:00:00.000Z'))).toBe(
      false
    );
    expect(isCalendarEventPast(start, end, false, new Date('2026-08-20T08:00:00.000Z'))).toBe(true);
  });
});
