...
export function LayoutGridEditor({
      ...
      <div className="space-y-2 p-2.5">
          ...
          {PALETTE_ORDER.map((id) => {
            ...
            return (
              <button
                ...
                className={`touch-manipulation rounded-full border px-2 py-0.5 text-[10px] transition-colors ${paletteId === id ? 'border-primary bg-primary text-primary-foreground' : 'border-border text-muted-foreground hover:bg-accent/50'}`}
              >
                {p.label}
              </button>
            );
        ...
        <div className="flex flex-wrap items-center gap-1">
          ...
          <button
            ...
            title="Default"
          >
            <div className="flex h-full w-full items-center justify-center bg-card/85">
              <svg
                width="12"
                height="12"
                viewBox="0 0 24 24"
        ...
        {target === 'text' && (
          <div className="flex items-center gap-1">
            <span className="w-10 shrink-0 text-[10px] text-muted-foreground">Size</span>
            {[
              { value: 0.75, label: 'S' },
              { value: 1, label: 'M' },
              { value: 1.25, label: 'L' },
              ...
              <button
                ...
                onPointerDown={(e) => e.stopPropagation()}
                className={`h-7 w-7 touch-manipulation rounded-full border text-[10px] transition-colors ${
            ...
            {isFrosted
              ? [
                  ...
                  <button
                    ...
                    onPointerDown={(e) => e.stopPropagation()}
                    className={`touch-manipulation rounded border px-2 py-1 text-[10px] transition-colors ${bgOpacity === v ? 'border-primary bg-primary text-primary-foreground' : 'border-border hover:bg-accent/50'}`}
                  >
                    {l}
                  </button>
                ))
              : [0, 0.25, 0.5, 0.75, 1].map((o) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={set}
                      onPointerDown={(e) => e.stopPropagation()}
                      className={`h-7 w-7 touch-manipulation rounded-full border text-[10px] transition-colors ${cur === o ? 'border-primary bg-primary text-primary-foreground' : 'border-border hover:bg-accent/50'}`}
                    >
                      {Math.round(o * 100)}%
                    </button>
                  );
  ...
  const renderPropertiesBar = () => {
    ...
    const olOpacity = selectedWidgetConfig.outlineOpacity ?? 1;
    const displayName =
      selectedWidgetConfig.i.charAt(0).toUpperCase() + selectedWidgetConfig.i.slice(1);
    const registryEntry = WIDGET_REGISTRY[selectedWidgetConfig.i];
    const widgetHasGrid = registryEntry?.hasGrid === true;

    return (
      <div
        className="relative z-[150] border-b border-border bg-card/95 backdrop-blur-sm"
        onPointerDown={(e) => e.stopPropagation()}
      >
                        ...
                        <div className="flex items-center gap-1">
                          {[0, 0.25, 0.5, 0.75, 1].map((o) => (
                            <button
                              ...
                              onPointerDown={(e) => e.stopPropagation()}
                              className={`h-8 w-8 touch-manipulation rounded-full border text-[10px] transition-colors ${gridLineOp === o ? 'border-primary bg-primary text-primary-foreground' : 'border-border hover:bg-accent/50'}`}
                            >
                              {Math.round(o * 100)}%
                            </button>
                          ))}
                          ...
                          <div className="mt-1.5 flex items-center gap-1">
                            ...
                            {[0, 0.1, 0.25, 0.5, 0.75, 1].map((o) => (
                              <button
                                ...
                                onPointerDown={(e) => e.stopPropagation()}
                                className={`h-7 w-7 touch-manipulation rounded-full border text-[9px] transition-colors ${cellBgOp === o ? 'border-primary bg-primary text-primary-foreground' : 'border-border hover:bg-accent/50'}`}
                              >
                                {Math.round(o * 100)}%
                              </button>
                            ))}
    ...
    return (
      ...
      >
        <div className="absolute" style={{ left: 0, top: 0, width: rectW, height: rectH }}>
          ...
          <span
            className="absolute rounded px-1 py-0.5 text-[10px] font-semibold"
            style={{
