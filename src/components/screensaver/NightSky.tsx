...
export function NightSky({ events, loading }: Props) {
  ...
  useEffect(() => {
    ...
    const paint = (timestamp: number) => {
      ...
      for (let index = 0; index < starCount; index++) {
        ...
        const x =
          (((hash(`x-${index}`) * width + drift * (8 + seed * 12)) % width) + width) % width;
      ...
      upcoming.forEach((event) =>
        groups.set(event.color, [...(groups.get(event.color) || []), event])
      );
      Array.from(groups.entries()).forEach(([color, group], groupIndex) => {
        ...
        const cy =
          height * (0.28 + Math.floor(groupIndex / 4) * 0.28) + Math.cos(drift + groupIndex) * 12;
        ...
        context.stroke();
        points.forEach((point, index) => {
          context.shadowBlur = night ? 5 : 12;
          context.shadowColor = color;
          context.fillStyle = color;
          context.beginPath();
          context.arc(point.x, point.y, index === 0 ? 3.5 : 2.4, 0, Math.PI * 2);
          context.fill();
        });
      ...
      const moonX = width * 0.86,
        moonY = height * 0.18,
        moonR = Math.min(width, height) * 0.044;
      ...
      if (comet) {
        ...
        const x = -120 + progress * (width + 240),
          y = height * 0.72 - progress * height * 0.22;
        const tail = context.createLinearGradient(x - 150, y + 45, x, y);
        tail.addColorStop(0, '#00000000');
        tail.addColorStop(1, `${comet.color}aa`);
        context.strokeStyle = tail;
        context.lineWidth = 3;
        context.beginPath();
        context.moveTo(x - 150, y + 45);
        context.lineTo(x, y);
        context.stroke();
        context.fillStyle = comet.color;
        context.shadowBlur = 18;
        context.shadowColor = comet.color;
        context.beginPath();
  ...
  return (
    ...
    >
      <canvas
        ref={canvasRef}
        className={`absolute inset-0 h-full w-full ${night ? 'brightness-[.42]' : ''}`}
        aria-hidden="true"
      />
      <div className="pointer-events-none absolute inset-x-[7vw] bottom-[8vh] flex items-end justify-between gap-10 text-white/90 [text-shadow:0_2px_22px_rgba(0,0,0,.9)]">
        <div>
          <time
            className="block text-[clamp(4rem,9vw,11rem)] font-extralight leading-none tracking-[-.06em]"
            dateTime={now.toISOString()}
          >
            {time}
          </time>
          <p className="mt-3 text-[clamp(1rem,1.4vw,2rem)] font-light tracking-[.12em] text-white/65">
            {date}
          </p>
        </div>
        <div className="max-w-[38vw] border-l border-white/20 pl-7 text-right" aria-live="polite">
          ...
          <p
            className="mt-3 text-[clamp(1.2rem,2.1vw,2.7rem)] font-light"
            style={{ color: next?.color || undefined }}
          >
            {loading ? 'Finding the next star…' : next ? next.title : 'The sky is clear'}
          </p>
          {next && (
            <p className="mt-2 text-[clamp(.9rem,1.1vw,1.35rem)] text-white/65">
      ...
      {comet && (
        <div className="pointer-events-none absolute left-1/2 top-[70%] -translate-x-1/2 rounded-full border border-white/15 bg-black/20 px-5 py-2 text-[clamp(.8rem,1vw,1.2rem)] font-light tracking-wide text-white/75 backdrop-blur-sm">
          Tomorrow ·{' '}
          <span style={{ color: comet.color }}>
            {comet.startTime.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })} ·{' '}
            {comet.title}
          </span>
        </div>
      )}
      {comet && (
        <div className="sr-only">
          Tomorrow&apos;s first event: {comet.title} at{' '}
          {comet.startTime.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}
        </div>
      )}
    </div>
