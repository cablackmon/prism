        ...
        {photos.map((photo, index) => {
          ...
          return (
            <div
              ...
              onClick={() => (selectMode ? onToggleSelect?.(photo.id) : onPhotoClick(index))}
            >
              ...
              {selectMode && (
                <span
                  className={cn(
                    'absolute left-1.5 top-1.5 z-10 flex h-6 w-6 items-center justify-center rounded-full border-2 transition-colors',
                    selected
              ...
              <span
                className={`absolute right-1.5 top-1.5 h-2.5 w-2.5 rounded-full ${qualityColors[getResolutionQuality(photo.width, photo.height)]} ring-1 ring-black/30`}
              />
              {/* Usage badge */}
              <span className="absolute bottom-1.5 left-1.5 rounded bg-black/50 px-1 py-0.5 text-[10px] font-bold leading-none text-white/80">
                {usageBadge(photo.usage)}
              </span>
              {/* Orientation badge */}
              <span className="absolute bottom-1.5 right-1.5 rounded bg-black/50 px-1 py-0.5 text-[10px] font-bold leading-none text-white/80">
